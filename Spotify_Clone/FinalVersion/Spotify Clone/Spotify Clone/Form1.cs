...
namespace Spotify_Clone
{
	public partial class Form1 : Form
	{
		...
		private void AuxForm2(object tag){try{atualizacao = false;try{if ((int)tag != 0){IdPlayList = (int)tag;atualizacao = true;}else{IdPlayList = 0;atualizacao = false;}}catch{MessageBox.Show("Your were very fast!! Take it easy, beacuse the program dont detect playList", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);return;}Form2 form2 = new Form2(atualizacao, IdPlayList);form2.ShowDialog();form2.Close();var caminhoficheiro = Path.GetDirectoryName(Assembly.GetEntryAssembly().Location);escrever();int a = 1;_listInformacoes.ToList().ForEach(item =>{item.IDList = a; a++;});	form2.Close();var json = JsonConvert.SerializeObject(_listInformacoes);File.WriteAllText(caminhoficheiro + "/Musicas.json", json);}catch{return;}Atualiza();}
		...
		private void timer1_Tick(object sender, EventArgs e)
		{
			...
			if (PBC.Value == PBC.Maximum)
			{
				try
				{
					...
					labelControl2.Text = NameMusic[NameMusic.Length - 1].Substring(0, (NameMusic[NameMusic.Length - 1].Count() - 4));axWindowsMediaPlayer1.Ctlcontrols.play();
					PBC.Value = 0;
					dt = 0;
				}
				catch
		...
		private void timer2_Tick(object sender, EventArgs e)
		{
			if (FormVideo == false)
			{
				...
				if (pE_PauseaPlay.Tag.ToString() == "1")
					dt = 0;
			}
			else
			{
				...
				if (Video.progresso != 0)PBC.Maximum = (int)
						Video.maxprogresso;
				if (axWindowsMediaPlayer1.URL != (_listInformacoes[(IdPlayList - 1)].Caminho_da_Musica).ToString())
				{
					NameMusic = _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica[(int)Ordem_de_Reproducao[int.Parse(labelControl2.Tag.ToString())]].Split(new string[] { "\\" }, StringSplitOptions.None);
					labelControl2.Text = NameMusic[NameMusic.Length - 1].Substring(0, (NameMusic[NameMusic.Length - 1].Count() - 4));
				...
				}if (PBC.Value == PBC.Maximum || PBC.Value == (PBC.Maximum - 1))
				{
		...
		#endregion
		#region panel
		private void pnl_Click(object sender, EventArgs e){Panel pnl = sender as Panel;IdPlayList = (int)pnl.Tag;MusicaPlay((int)pnl.Tag - 1);}
		private void painel_MouseHover(object sender, EventArgs e){Panel pnl = sender as Panel;TSMIRemove.Tag = (int)pnl.Tag;TSMIEditPlayList.Tag = (int)pnl.Tag;var aux = (int)pnl.Tag;}
		#endregion
		#region lbl
