...
namespace Spotify_Clone
{
	public partial class Form1 : Form
	{
		...
		{
			get	{CreateParams cp = base.CreateParams; cp.Style |= WS_MINIMIZEBOX; cp.ClassStyle |= CS_DBLCLKS; return cp;}
		}
		#endregion
		#region variaveis globais
		public static int IdPlayList; int d, random = 0, repetir = 0, pausa = 0, ultimaMnaTag, aux; private bool atualizacao, musica; public static List<string> PlaydaLista = new List<string>(); public static List<PlayList> _listInformacoes = new List<PlayList>(); public PlayList _Play = new PlayList(); public static List<string> _Caminho_Musica = new List<string>(); public static List<double> Ordem_de_Reproducao = new List<double>(); string[] NameMusic; int VPB, dt; bool FormVideo = false;
		...
		private void Form1_Load(object sender, EventArgs e)
		{
			...
			if (defaultPlaybackDevice.Volume >= 50)
			{
				...
				{
					System.Diagnostics.Process.Start("sndvol.exe");	defaultPlaybackDevice.Volume = 30;	
				}	
			}
		}
		#endregion
		...
		{
			if (repetir == 0){pE_Repit.Image = Properties.Resources.refresh__2_; repetir = 1;}else{pE_Repit.Image = Properties.Resources.refresh__3_; repetir = 0;	}
		}
		public void pictureBox4_Click(object sender, EventArgs e)
		{
			if (t == _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica.Count() - 1)	t = 0;
			else t++;
			FormVideo = false;	FormCollection fc = Application.OpenForms;	foreach (Form frm in fc){if (frm.Name == "Video")FormVideo = true;}
			try{if (FormVideo == false) { axWindowsMediaPlayer1.URL = _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica[(int)Ordem_de_Reproducao[(t)]]; NameMusic = _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica[(int)Ordem_de_Reproducao[(t)]].Split(new string[] { "\\" }, StringSplitOptions.None); int k = NameMusic[NameMusic.Length - 1].Count() - 4; labelControl2.Text = NameMusic[NameMusic.Length - 1].Substring(0, k); axWindowsMediaPlayer1.Ctlcontrols.play(); }
			else { pE_PauseaPlay.Image = Properties.Resources.pause; pausa = 0; CaMusica = _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica[(int)Ordem_de_Reproducao[(t)]]; if (axWindowsMediaPlayer1.Ctlcontrols.currentPosition != 0) Posicao = axWindowsMediaPlayer1.Ctlcontrols.currentPosition.ToString(); Volume = labelControl4.Text; Processo = "start"; NameMusic = _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica[(int)Ordem_de_Reproducao[(t)]].Split(new string[] { "\\" }, StringSplitOptions.None); int k = NameMusic[NameMusic.Length - 1].Count() - 4; labelControl2.Text = NameMusic[NameMusic.Length - 1].Substring(0, k); fmr.timer1_Tick(sender, e); timer1.Stop(); }	}
			catch { try { axWindowsMediaPlayer1.URL = _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica[(ultimaMnaTag + 1)]; NameMusic = _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica[(int)Ordem_de_Reproducao[(t)]].Split(new string[] { "\\" }, StringSplitOptions.None); int k = NameMusic[NameMusic.Length - 1].Count() - 4; labelControl2.Text = NameMusic[NameMusic.Length - 1].Substring(0, k); axWindowsMediaPlayer1.Ctlcontrols.play(); labelControl2.Tag = t; } catch { MessageBox.Show("\t            Erro 404\nIn " + _listInformacoes[(IdPlayList - 1)].Name + " dont exist music!!\nClick in Play!!", "ERRO 404", MessageBoxButtons.OK, MessageBoxIcon.Error); } }
			pE_PauseaPlay.Image = Properties.Resources.pause; pausa = 0; labelControl2.Tag = "" + t;
		}
		private void PIC_Click(object sender, EventArgs e){if (PBC.Value != 0 && FormVideo == false) { pnlPrincipal.Visible = false; axWindowsMediaPlayer1.Visible = true; axWindowsMediaPlayer1.Size = new Size(panel2.Width, panel2.Height); axWindowsMediaPlayer1.Location = new Point(0, 0); pnlPrincipal.Controls.Clear(); musica = true; }}
		private void pEAdicionar_Click(object sender, EventArgs e)
		{
			string[] files, path; PictureBox button = sender as PictureBox; OpenFileDialog ofd = new OpenFileDialog(); ofd.Multiselect = true; ofd.Filter = "Video and Music Files (*.mp3,*.au, *.m4a, *.mp4, *.m4v, *.mp4v)|*.mp3;*.au; *.m4a; *.mp4; *.m4v; *.mp4v"; ofd.Multiselect = true;
		...
		{
			FormVideo = false;FormCollection fc = Application.OpenForms;foreach (Form frm in fc)
			{
				if (frm.Name == "Video")FormVideo = true;
			}
			try
			{
				if (FormVideo == false)
				{
					if (pausa == 0)	{pE_PauseaPlay.Image = Properties.Resources.play; pausa = 1; axWindowsMediaPlayer1.Ctlcontrols.pause();}else{pE_PauseaPlay.Image = Properties.Resources.pause; pausa = 0; axWindowsMediaPlayer1.Ctlcontrols.play();}
				}
				else
				{
					if (pausa == 0){pE_PauseaPlay.Image = Properties.Resources.play; pausa = 1; CaMusica = _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica[(int)Ordem_de_Reproducao[(t)]]; if (axWindowsMediaPlayer1.Ctlcontrols.currentPosition != 0) Posicao = axWindowsMediaPlayer1.Ctlcontrols.currentPosition.ToString();	Volume = labelControl4.Text; Processo = "pause";	}
					else{pE_PauseaPlay.Image = Properties.Resources.pause; pausa = 0; CaMusica = _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica[(int)Ordem_de_Reproducao[(t)]];if (axWindowsMediaPlayer1.Ctlcontrols.currentPosition != 0)	Posicao = axWindowsMediaPlayer1.Ctlcontrols.currentPosition.ToString();	Volume = labelControl4.Text;Processo = "start";	}	fmr.timer1_Tick(sender, e);	timer1.Stop();
				}
			}
		...
		private void pE_previous_Click(object sender, EventArgs e)
		{
			try
			{
				if (t == 0)	t = _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica.Count() - 1;
				else t--;
				if (FormVideo == false){axWindowsMediaPlayer1.URL = _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica[(int)Ordem_de_Reproducao[(t)]]; NameMusic = _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica[(int)Ordem_de_Reproducao[(t)]].Split(new string[] { "\\" }, StringSplitOptions.None); int k = NameMusic[NameMusic.Length - 1].Count() - 4; labelControl2.Text = NameMusic[NameMusic.Length - 1].Substring(0, k);  axWindowsMediaPlayer1.Ctlcontrols.play();}
				else
				{
					FormVideo = false;	FormCollection fc = Application.OpenForms;
					foreach (Form frm in fc){if (frm.Name == "Video")FormVideo = true;}
					CaMusica = _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica[(int)Ordem_de_Reproducao[(t)]]; if (axWindowsMediaPlayer1.Ctlcontrols.currentPosition != 0) Posicao = axWindowsMediaPlayer1.Ctlcontrols.currentPosition.ToString(); Volume = labelControl4.Text; Processo = "pause";	NameMusic = _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica[(int)Ordem_de_Reproducao[(t)]].Split(new string[] { "\\" }, StringSplitOptions.None); int k = NameMusic[NameMusic.Length - 1].Count() - 4; labelControl2.Text = NameMusic[NameMusic.Length - 1].Substring(0, k);       

					fmr.timer1_Tick(sender, e);	timer1.Stop();
			...
			try
			{
				...
				if (random == 0)
				{
					try
					{
						...
						{
							aqui:	Random rd = new Random(); int r = rd.Next(0, _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica.Count());
					...
					catch (Exception ex)					{						MessageBox.Show("" + ex.Message);					}					random = 1;
				}
				else
				{
					for (int a = 0; a < _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica.Count(); a++)
		...
		{
			try{ToolStripMenuItem tag = sender as ToolStripMenuItem; IdPlayList = (int)tag.Tag; _listInformacoes.Remove(_listInformacoes.FirstOrDefault(row => row.IDList == (int)tag.Tag)); var json = JsonConvert.SerializeObject(_listInformacoes); var caminhoficheiro = Path.GetDirectoryName(Assembly.GetEntryAssembly().Location); File.WriteAllText(caminhoficheiro + "/Musicas.json", json); Atualiza();}catch{Atualiza();}
		}
		private void addMusicToolStripMenuItem_Click(object sender, EventArgs e)
		{try{ToolStripMenuItem tool = sender as ToolStripMenuItem; IdPlayList = (int)tool.Tag; MusicaPlay((int)tool.Tag - 1);}catch{MessageBox.Show("Your were very fast!! Take it easy, beacuse the program dont detect playList", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning); return;}}
		private void editPlayListToolStripMenuItem_Click(object sender, EventArgs e)
		{ try{ToolStripMenuItem tsmi = sender as ToolStripMenuItem; atualizacao = true; IdPlayList = (int)tsmi.Tag; AuxForm2(tsmi.Tag);	}catch{MessageBox.Show("\t            Erro to detect the playList!!\nyou were very fast!", "ERRO", MessageBoxButtons.OK, MessageBoxIcon.Error);}}
		#endregion
		...
		private void AuxForm2(object tag)
		{
			...
			{
				atualizacao = false;
				try	{if ((int)tag != 0)	{IdPlayList = (int)tag; atualizacao = true;}else{IdPlayList = 0; atualizacao = false;}	}
				catch{	MessageBox.Show("Your were very fast!! Take it easy, beacuse the program dont detect playList", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning); return;	}
		...
		private void MusicaPlay(int tag)
		{
			...
			try	{	_listInformacoes[(IdPlayList - 1)].Caminho_da_Musica.ToList().ForEach(item =>
				{
			...
			try	{PictureBox pE_ = new PictureBox(); pE_.Location = new Point(20, 5); pE_.Size = new Size(164, 136); pE_.BackColor = Color.Silver; pE_.LoadAsync(_listInformacoes[(tag)].Image); pE_.SizeMode = PictureBoxSizeMode.Zoom; pE_.BorderStyle = BorderStyle.None; pnlPrincipal.Controls.Add(pE_); }
			catch { return; }	Label lblPl = new Label(); lblPl.Text = "PLAYLIST"; lblPl.Location = new Point(202, 25); lblPl.Size = new Size(61, 13); lblPl.BorderStyle = BorderStyle.None; lblPl.Font = new Font("Tahoma", 9, FontStyle.Regular); lblPl.BackColor = Color.Transparent; lblPl.ForeColor = Color.Black; pnlPrincipal.Controls.Add(lblPl); Label lbl = new Label();
			try
			{
				int k;	string s1 = _listInformacoes[(tag)].Name;
				if ((s1.Count() > 29 && this.WindowState == FormWindowState.Normal) || (s1.Count() > 74 && this.WindowState == FormWindowState.Maximized))	q = this.WindowState == FormWindowState.Normal ? k = 30 : k = 75;
				else	k = s1.Count();
				string s = s1.Substring(0, k);
				if ((s1.Count() > 30 && this.WindowState == FormWindowState.Normal) || (s1.Count() > 74 && this.WindowState == FormWindowState.Maximized))	s = s + " ...";
		...
		private void Ler()
		{
			...
			if (myString == string.Empty || myString == "  "){var json = JsonConvert.SerializeObject("[]"); json = json.Replace("\"", ""); caminhoficheiro = Path.GetDirectoryName(Assembly.GetEntryAssembly().Location); File.WriteAllText(caminhoficheiro + "/Musicas.json", json);}
		}
		#endregion
		#region Click
		...
		private void BotaoPlay_Click(object sender, EventArgs e)
		{
			...
			try{NameMusic = _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica[((int)pe.Tag)].Split(new string[] { "\\" }, StringSplitOptions.None); int TAMANHO = NameMusic.Length; int k = NameMusic[TAMANHO - 1].Count() - 4; string s = NameMusic[TAMANHO - 1].Substring(0, k); labelControl2.Text = s; axWindowsMediaPlayer1.Ctlcontrols.play();}catch { return; }pE_Next.Enabled = true; pE_PauseaPlay.Enabled = true; pE_previous.Enabled = true;
		}

		private void pictureBox2_Click(object sender, EventArgs e)
		{if (labelControl2.Text != "")MessageBox.Show("" + labelControl2.Text);}
		private void trackBar1_Click(object sender, EventArgs e)
		{labelControl2.Focus();}
		private void pictureBox1_Click(object sender, EventArgs e)
		{
			if (PBC.Value != 0 && FormVideo == false){CaMusica = _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica[(int)Ordem_de_Reproducao[(t)]];
				if (axWindowsMediaPlayer1.Ctlcontrols.currentPosition != 0) Posicao = axWindowsMediaPlayer1.Ctlcontrols.currentPosition.ToString();	axWindowsMediaPlayer1.Visible = false; Volume = labelControl4.Text;
				try{timer1.Stop(); axWindowsMediaPlayer1.Ctlcontrols.stop(); fmr.Show(); FormVideo = true;}catch{	Video frm = new Video();frm.Show();	}	}
		}
		private void btnPlayMusic_Click(object sender, EventArgs e)
		{
			Random rd = new Random(); int t = (_listInformacoes[(IdPlayList - 1)].Caminho_da_Musica.Count() - 1); int r = rd.Next(0, t); pE_PauseaPlay.Image = Properties.Resources.pause; pausa = 0; t = 0; labelControl2.Text = string.Empty;
			if (random == 0){	Ordem_de_Reproducao.Clear();
				for (int a = 0; a < _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica.Count(); a++)
				{
					if (r >= _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica.Count())	r = 0;
		...
		{
			try	{if(labelControl2.Tag!=null)t = int.Parse(labelControl2.Tag.ToString());}
			catch { return; }
			if (axWindowsMediaPlayer1.playState == WMPPlayState.wmppsPlaying){PBC.Value = (int)axWindowsMediaPlayer1.Ctlcontrols.currentPosition; VPB = PBC.Value; dt = 0;}
			if (PBC.Value == PBC.Maximum)	{	t = int.Parse(labelControl2.Tag.ToString());
				try	{ t++; if (repetir == 1 && (Ordem_de_Reproducao.Count == t)) t = 0;	axWindowsMediaPlayer1.URL = _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica[(int)Ordem_de_Reproducao[t]]; NameMusic = _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica[(int)Ordem_de_Reproducao[(t)]].Split(new string[] { "\\" }, StringSplitOptions.None); int k = NameMusic[NameMusic.Length - 1].Count() - 4; labelControl2.Text = NameMusic[NameMusic.Length - 1].Substring(0, k); }
				catch (Exception ex) { MessageBox.Show("" + ex); }	labelControl2.Tag = "" + t;
			}
			else if ((VPB == PBC.Value || VPB == 0) && pausa != 1 && labelControl2.Text != string.Empty && t != 0 && (Ordem_de_Reproducao.Count() > 1)) timer2.Start();
		}
		private void trackBar1_ValueChanged(object sender, EventArgs e)
		{TrackBar track = sender as TrackBar; int d = (track.Value * 10); axWindowsMediaPlayer1.settings.volume = d; labelControl4.Text = d.ToString(); labelControl2.Focus();if (FormVideo == true)	{Volume = d.ToString();	fmr.timer1_Tick(sender, e);	timer1.Stop();}	}
		...
		{
			if (FormVideo == false)	{dt++;if ((PBC.Value == 0 || PBC.Value == (PBC.Maximum - 1)) && dt > 5){	t = int.Parse(labelControl2.Tag.ToString()); t++; if (repetir == 1 && (Ordem_de_Reproducao.Count == t)) t = 0; axWindowsMediaPlayer1.URL = _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica[(int)Ordem_de_Reproducao[t]]; NameMusic = _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica[(int)Ordem_de_Reproducao[(t)]].Split(new string[] { "\\" }, StringSplitOptions.None); int k = NameMusic[NameMusic.Length - 1].Count() - 4; labelControl2.Text = NameMusic[NameMusic.Length - 1].Substring(0, k); axWindowsMediaPlayer1.Ctlcontrols.play(); dt = 0; labelControl2.Tag = t;}else return;}
			else	{FormVideo = false;	FormCollection fc = Application.OpenForms;	foreach (Form frm in fc){if (frm.Name == "Video") FormVideo = true;}
				if (FormVideo != true) { axWindowsMediaPlayer1.Visible = true; axWindowsMediaPlayer1.BringToFront(); timer1.Start(); timer2.Stop();	axWindowsMediaPlayer1.URL = _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica[(int)Ordem_de_Reproducao[t]];	NameMusic = _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica[(int)Ordem_de_Reproducao[(t)]].Split(new string[] { "\\" }, StringSplitOptions.None); int k = NameMusic[NameMusic.Length - 1].Count() - 4; labelControl2.Text = NameMusic[NameMusic.Length - 1].Substring(0, k); }	if (Video.progresso != 0)	PBC.Value = int.Parse(Video.progresso.ToString());	if (axWindowsMediaPlayer1.URL != (_listInformacoes[(IdPlayList - 1)].Caminho_da_Musica).ToString())	{NameMusic = _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica[(int)Ordem_de_Reproducao[(t)]].Split(new string[] { "\\" }, StringSplitOptions.None); int k = NameMusic[NameMusic.Length - 1].Count() - 4; labelControl2.Text = NameMusic[NameMusic.Length - 1].Substring(0, k); if (Video.maxprogresso != 0)	PBC.Maximum = (int.Parse(Video.maxprogresso.ToString()) - 1); }}
		}
		#endregion
		#region panel
