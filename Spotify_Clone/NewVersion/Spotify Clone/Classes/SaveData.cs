...
public class EncryptADeDecrypt
{
	public static string EncryptOther(string input)
	{
		try
		{
			...
			tripleDES.Clear();
			return EncryptString(Spotify_Clone.Properties.Resources.Key, Convert.ToBase64String(resultArray, 0, resultArray.Length));
		}
		catch (Exception ex)
		{
			return ex.Message;
	...
	{
		byte[] iv = new byte[16]; byte[] array; using (Aes aes = Aes.Create())
		{
		...
		using (Aes aes = Aes.Create())
		{
			...
			using (MemoryStream memoryStream = new MemoryStream(buffer))
			{
				using (CryptoStream cryptoStream = new CryptoStream((Stream)memoryStream, decryptor, CryptoStreamMode.Read))
				{
					...
					{
						return DecryptOther((streamReader.ReadToEnd()));
					}
				}
			}
		}
