...
namespace Spotify_Clone
{
	public partial class Form1 : Form
	{
		...
		{
			if (e.Button == MouseButtons.Left && FormVideo == false)
			{
				ReleaseCapture();
		...
		private void backgroundWorker1_DoWork(object sender, DoWorkEventArgs e)
		{
			...
			{
				Environment.CurrentDirectory = Environment.GetEnvironmentVariable("temp");
				var myString = File.ReadAllText(Environment.CurrentDirectory + "/Musicas.json");
				if (myString == "\"[]\"")
				{
			...
			catch (Exception ex)
			{
				...
				File.CreateText(Environment.CurrentDirectory + "Musicas.json");
				MessageBox.Show("I'm create new paste,see there,if you don't lose this document again!!!!", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
			}
		}
		...
		private void removeThePlayListToolStripMenuItem_Click(object sender, EventArgs e)
		{
			try
			{
				...
				if (pictureBox2.Tag != null)
					if (int.Parse(pictureBox2.Tag.ToString()) == (int)tag.Tag)
					{
						PBC.Value = 0;
						labelControl2.Text = "";
						labelControl2.Tag = "";
						...
						pE_Random.Tag = 0;
						labelControl4.Text = 50.ToString();
						pE_PauseaPlay.Tag = 0; pE_PauseaPlay.Image = Properties.Resources.play; pE_Random.Image = Properties.Resources.change;
		...
		private void AuxForm2(object tag)
		{
			try
			{
				...
				Form2 form2 = new Form2(atualizacao, IdPlayList);
				form2.ShowDialog();
				form2.Close();
				escrever();
				int a = 1;
