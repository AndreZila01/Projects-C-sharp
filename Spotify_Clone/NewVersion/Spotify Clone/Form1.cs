	...
	public partial class Form1 : Form
	{
		...
		private void btnPlayMusic_Click(object sender, EventArgs e)
		{
			...
			if (pE_Random.Tag.ToString() == "0")
			{
				...
				for (int a = 0; a < _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica.Count();
				a++)
				{
		...
		private void ToolStripMenuItem_Click(object sender, EventArgs e)
		{
			switch (((ToolStripMenuItem)sender).Name)
			{
				case "TSMIRemove":
					{
						try
						{
							if (picInfos.Tag != null) if (int.Parse(picInfos.Tag.ToString()) == (int)((ToolStripMenuItem)sender).Tag)
								{
									labelControl2.Text = "";
									labelControl2.Tag = "";
									...
									if (FormVideo == true)
									{
										...
										{
											if (frm.Name == "Video") frm.Close();
										}
									}
								}
							if (picForm3.Tag != null) if ((int.Parse(picForm3.Tag.ToString())) == (((int)((ToolStripMenuItem)sender).Tag)))
								{
									labelControl2.Text = "";
									IdPlayList = 0;
									PBC.Value = 0;
							...
							if (txtPaths.Text == "%appdata%") txtPaths.Text = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData);
				...
				case "TSMIPause":
					...
					break;
				case "TSMINext":
					if (int.Parse(labelControl2.Tag.ToString()) == _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica.Count() - 1) labelControl2.Tag = "" + 0;
				...
				case "pE_previous":
					{
						try
						{
							if (int.Parse(labelControl2.Tag.ToString()) == 0) labelControl2.Tag = "" + (_listInformacoes[(IdPlayList - 1)].Caminho_da_Musica.Count() - 1);
							else labelControl2.Tag = (int.Parse(labelControl2.Tag.ToString())) - 1;
							if (FormVideo == false)
							{
								axwindows();
							...
							pE_PauseaPlay.Image = Properties.Resources.pause;
							pE_PauseaPlay.Tag = "0";
							Discord(labelControl2.Text, int.Parse(axWindowsMediaPlayer1.Ctlcontrols.currentPosition.ToString()), Convert.ToInt32(axWindowsMediaPlayer1.Ctlcontrols.currentItem.duration));
							dt = 0;
							PBC.Value = 0;
						}
						catch (Exception ex)
		...
		private void label11_Click(object sender, EventArgs e)
		{
			...
			if (((Label)sender).Name == "lblDuracao" || ((Label)sender).Name == "lblNomeDisc" || ((Label)sender).Name == "lblSO") SaveSettings(null);
		}
		...
		private void bgwCarregar_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
		{
			...
			if (this.Tag.ToString() == "MusicaPlay" || this.Tag.ToString() == "MusicaPlayD")
			{
				...
				try
				{
					...
					{
						if (this.Tag.ToString() == "MusicaPlayD") limite = _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica.Count();
						else limite = 15;
						PlaydaLista.ToList().ForEach(Testes =>
						{
							...
							if (d <= limite)
							{
								...
								if (d == 5) pnl1.Size = new Size(pnl.Width - 31, 40);
								else if (d > 5) pnl1.Size = new Size(pnl.Width - 29, 40);
								else pnl1.Size = new Size(pnl.Width - 14, 40);
								pnl1.Anchor = AnchorStyles.Top | AnchorStyles.Right | AnchorStyles.Left;
								pnl1.BackColor = Color.FromArgb(224, 224, 209);
								if (d > 1) pnl1.Location = new Point(5, d * 46);
								else if (d == 1) pnl1.Location = new Point(5, 48);
								else pnl1.Location = new Point(5, 3);
			...
			else if (this.Tag.ToString() == "AddPnl")
			{
				pnlPrincipal.Visible = true;
				axWindowsMediaPlayer1.Visible = false;
		...
		private void bgwCarregar_DoWork(object sender, DoWorkEventArgs e)
		{
			if (this.Tag.ToString() != "MusicaPlay")
			{
				try
				{
					if (System.IO.File.Exists(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + "/helper.txt")) txtPaths.Text = System.IO.File.ReadAllText(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + "/helper.txt");
					Backend bk = new Backend();
					try
					{
		...
		public void Discord(string NomeMusic, int tempoatual, int duracao)
		{
			if (chkDiscord.Checked)
			{
				if (!switchNome.Checked) NomeMusic = "catJAM catJAM catJAM catJAM ";
				if (client == null)
				{
					...
					panel4.Tag = key + " + ";
					{
						try
						{
							...
							keybind.ToList().ForEach(item =>
							{
								try
								{
									if ((ant[temp] == item.Replace(" ", string.Empty) && (keybind.Length) == ant.Length) && btnAnte == true) btnAnte = true;
									else btnAnte = false;
								}
								catch
								{
								...
								try
								{
									if ((pau[temp] == item.Replace(" ", string.Empty) && (keybind.Length) == pau.Length) && btnPause == true) btnPause = true;
									else btnPause = false;
								}
								catch
								{
								...
								try
								{
									if ((nex[temp] == item.Replace(" ", string.Empty) && (keybind.Length) == nex.Length) && btnNexts == true) btnNexts = true;
									else btnNexts = false;
								}
								catch
								{
		...
		private void SaveSettings(Settings settings)
		{
			if (settings == null)
			{
				...
				var sds = btnAnterior.Tag.ToString().Split('+').ToList().OrderBy(xs => xs);

				btnAnterior.Tag = "";

				sds.ToList().ForEach(item =>
				{
					if (btnAnterior.Tag.ToString() == "")
						btnAnterior.Tag += item;
					else
						btnAnterior.Tag += "+" + item;
				});

				sds = btnPausa.Tag.ToString().Split('+').ToList().OrderBy(xs => xs);

				btnPausa.Tag = "";

				sds.ToList().ForEach(item =>
				{
					if (btnPausa.Tag.ToString() == "")
						btnPausa.Tag += item;
					else
						btnPausa.Tag += "+" + item;
				});

				sds = btnNext.Tag.ToString().Split('+').ToList().OrderBy(xs => xs);

				btnNext.Tag = "";

				sds.ToList().ForEach(item =>
				{
					if (btnNext.Tag.ToString() == "")
						btnNext.Tag += item;
					else
						btnNext.Tag += "+" + item;
				});
				List<Settings> st = back.UpdateSettings(comboBox1.Text, SO, Discord, Enviar, Duracao, Atalho, Mini, NotifMusic, btnAnterior.Tag.ToString(), btnPausa.Tag.ToString(), btnNext.Tag.ToString(), txtPaths.Text, Xampp);
			...
			else
			{
				try
				{
					...
					{
						btnAnterior.Text = settings.Atalhos[0].MusicaAnterior.Replace("KEY_", " ");
						btnAnterior.Tag = settings.Atalhos[0].MusicaAnterior;
						btnPausa.Text = settings.Atalhos[0].Pausa.Replace("KEY_", " ");
						btnPausa.Tag = settings.Atalhos[0].Pausa;
						btnNext.Text = settings.Atalhos[0].MusicaSeguinte.Replace("KEY_", " ");
						btnNext.Tag = settings.Atalhos[0].MusicaSeguinte;
					}
					if (settings.atal != 1)
					{
						keyboardHook.KeyUp -= KeyboardHook_KeyUp;
			...
			if (txtXAMPP.Text != "") picXAMPP.Invoke((MethodInvoker)(() => picXAMPP.Visible = true));
		}
		...
		private void KeyboardHook_KeyUp(KeyboardHook.VKeys key)
		{
			if (labelControl2.Text != "" && pnlSettings.Visible == false && !panel7.Tag.ToString().Contains(key.ToString().Replace("LCONTROL", "CONTROL").Replace("LCONTROL", "CONTROL")))
			{
				...
				int keybind = bk.Atalho(ds, panel7.Tag.ToString());
				if (keybind != -1)
					if (keybind >= 0 && keybind < 3)
					{
						if (keybind == 0)
							EventosPE("pE_previous");
						else if (keybind == 1)
		...
		private void pE_Click(object sender, EventArgs e)
		{
			...
			switch (pE.Name)
			{
				case "pctPaths":
					...
					if (txtPaths.Text == "%appdata%") fsd.InitialDirectory = paths;
					if (fsd.ShowDialog(IntPtr.Zero)) if (!fsd.FileName.Contains("" + paths))
						{
							if (System.IO.File.Exists(paths + "/SpotifyClone/Musics.json")) System.IO.File.Delete(paths + "/SpotifyClone/Musics.json");
							if (System.IO.File.Exists(paths + "/SpotifyClone/Settings.json")) System.IO.File.Delete(paths + "/SpotifyClone/Settings.json");
							if (Directory.Exists(paths + "/SpotifyClone")) Directory.Delete(paths + "/SpotifyClone");
				...
				case "pE_minimizar":
					...
					break;
				case "picSettings":
					if (pnlSettings.Visible == true) SaveSettings(null);
				...
				case "pE_MaxMin":
					...
					foreach (var screen in Screen.AllScreens) lstemp.Add(screen);
				...
				case "pctXAMPP":
					...
					if ((ofds.ShowDialog() == DialogResult.OK) && ofds.SafeFileName.Contains("xampp-control.exe"))
					{
						...
						foreach (string ext in pathsss) if (ext != pathsss[(pathsss.Length - 1)]) txtXAMPP.Tag += ext + '\\';
				...
				case "picXAMPP":
					...
					Process.Start($"http://{myIP}/Spotify_Clone_Music/Spotify Clone.html");
					break;
				case "picBotaoPlay":
				...
				case "picForm3":
					{
						if (labelControl2.Text != "" && FormVideo == false)
						{
							...
							foreach (var screen in Screen.AllScreens) lstemps.Add(screen);
							ecra = (lstemps.FindIndex(dsssss => dsssss.DeviceName == scr.DeviceName));
							if (Ordem_de_Reproducao.Count() > 1) CaMusica = _listInformacoes[(IdPlayList - 1)].Caminho_da_Musica[(int)Ordem_de_Reproducao[int.Parse(labelControl2.Tag.ToString())]];
							else CaMusica = axWindowsMediaPlayer1.URL;
							axWindowsMediaPlayer1.Visible = false;
							Volume = labelControl4.Text;
							try
							{
		...
		private void btnAnterior_KeyDown(object sender, KeyEventArgs e)
		{
			...
			if (KeyboardHook.ConvertToBindable(e) != KeyboardHook.VKeys.UNKNOWN_KEY)
				if (!((System.Windows.Forms.Button)sender).Tag.ToString().Contains("" + KeyboardHook.ConvertToBindable(e).ToString().Replace("LCONTROL", "CONTROL").Replace("LCONTROL", "CONTROL")))
					if (((System.Windows.Forms.Button)sender).Text == "")
					{
						((System.Windows.Forms.Button)sender).Text += "" + KeyboardHook.ConvertToBindable(e).ToString().Replace("KEY_", " ");
						((System.Windows.Forms.Button)sender).Tag += "" + KeyboardHook.ConvertToBindable(e);
					}
					else
					{
						((System.Windows.Forms.Button)sender).Text += "+" + KeyboardHook.ConvertToBindable(e).ToString().Replace("KEY_", " ");
			...
			if ((bk.comparar(btnAnterior.Tag.ToString(), btnPausa.Tag.ToString(), btnNext.Tag.ToString())) == true)
				((System.Windows.Forms.Button)sender).FlatAppearance.BorderColor = Color.Red;
			else
		...
		private void backgroundWorker1_DoWork(object sender, DoWorkEventArgs e)
		{
			...
			if (pE_Random.Tag.ToString() == "0")
			{
				icnNotification.ShowBalloonTip(25, conf[comboBox1.Items.IndexOf(comboBox1.Text)].form1[0].ICNRANDOMtitulo, conf[comboBox1.Items.IndexOf(comboBox1.Text)].form1[0].RANDOMmensagemantes, ToolTipIcon.Info);
				try
				{
				...
				icnNotification.ShowBalloonTip(25, conf[comboBox1.Items.IndexOf(comboBox1.Text)].form1[0].ICNRANDOMtitulo, conf[comboBox1.Items.IndexOf(comboBox1.Text)].form1[0].RANDOMmensagemdepois, ToolTipIcon.Info);
				pE_Random.Tag = "1";
	   ...
	   a++) Ordem_de_Reproducao.Add(a);
		...
		private void timer2_Tick(object sender, EventArgs e)
		{
			...
			else
			{
				...
				if (Video.progresso != 0) PBC.Maximum = (int)Video.maxprogresso;
				if (Ordem_de_Reproducao.Count > 0)
				{
					...
					if (CaMusica != (_listInformacoes[(IdPlayList - 1)].Caminho_da_Musica[(int)Ordem_de_Reproducao[int.Parse(labelControl2.Tag.ToString())]]).ToString())
					{
						...
						labelControl2.Text = NameMusic[NameMusic.Length - 1].Substring(0, (NameMusic[NameMusic.Length - 1].Count() - 4));
						if (Video.maxprogresso != 0) PBC.Maximum = (int)Video.maxprogresso;
					}
					if (PBC.Value == 1) dt = 0;
				...
				try
				{
					if ((int)Video.progresso != 0) PBC.Value = (int)Video.progresso;
				}
				catch { }
				if (Video.info != "" && Video.info != null) EventosPE(Video.info);
				timer1.Start();
				Discord(labelControl2.Text, (int)(Video.progresso), (int)(Video.maxprogresso));
			}
		}
	}
}